/**
 * Baofeng UV-5RM channel data model and validation.
 * Shared between main process (protocol) and renderer (UI).
 */
...
/** Duplex modes */
export const DUPLEX_MODES = ["none", "+", "-", "split"] as const;

/** Tone modes */
export const TONE_MODES = ["None", "CTCSS", "DCS"] as const;

/** Power levels */
export const POWER_LEVELS = ["High", "Low"] as const;

/** Bandwidth options */
export const BANDWIDTH_OPTIONS = ["Wide", "Narrow"] as const;
...
export interface ChannelData {
  ...
  rxFreq: number;
  /** Transmit frequency in MHz (for split) or offset frequency */
  txFreq: number;
  /** Duplex mode */
  duplex: string;
  /** Tone mode */
...
/**
 * Validate a frequency against UV-5RM band limits.
 */
export function validateFrequency(freqMhz: number): BandValidation {
...
/**
 * Validate a complete channel entry. Returns warning strings.
 */
export function validateChannel(channel: ChannelData): string[] {
 ...
 */
export function createDefaultChannel(index: number): ChannelData {
  return {
    index,
    rxFreq: 0,
    txFreq: 0,
 ...
 */
export function createDefaultChannels(): ChannelData[] {
  return Array.from({ length: 128 }, (_, i) => createDefaultChannel(i));
}

/**
 * Find CTCSS tone code index for a frequency.
 */
export function ctcssToCode(freqHz: number): number {
 ...
 */
export function codeToCtcss(code: number): number {
  return code < CTCSS_TONES.length ? CTCSS_TONES[code] : 0;
}

/**
 * Find DCS code index for a DCS number.
 */
export function dcsToCode(dcsCode: number): number {
  const idx = DCS_CODES.indexOf(dcsCode);
  return idx >= 0 ? idx : 0;
}

/**
 * Find DCS number for a code index.
 */
export function codeToDcs(index: number): number {
 ...
 */
export function channelsToCSV(channels: ChannelData[]): string {
 ...
 */
export function channelsFromCSV(csv: string): ChannelData[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const duplex = (DUPLEX_MODES as readonly string[]).includes(cols[3]) ? cols[3] : "none";
    const toneMode = (TONE_MODES as readonly string[]).includes(cols[4]) ? cols[4] : "None";
    ...
    const power = (POWER_LEVELS as readonly string[]).includes(cols[8]) ? cols[8] : "High";
    const bandwidth = (BANDWIDTH_OPTIONS as readonly string[]).includes(cols[9]) ? cols[9] : "Wide";
